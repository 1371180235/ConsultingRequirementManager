...
def main():
    ...
    try:
        ...
        database.log("系统", "healthcheck", None, "check", "", "success", "本地版部署健康检查通过")
        result = {
            "status": "ok",
        ...
        app.LOGGER.exception("healthcheck_failed variant=sqlite")
        if database is not None:
            try:
                database.log("系统", "healthcheck", None, "check", "", "failed",
                             "本地版部署健康检查失败", result="failed")
            except Exception:
                app.audit_event("系统", "healthcheck", None, "check", "本地版部署健康检查失败", result="failed")
