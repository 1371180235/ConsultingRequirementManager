...
def default_install_dir() -> Path:
    return Path.home() / "Documents" / "ConsultingRequirementManagerRemote"
...
def _copy_if_missing(source: Path, destination: Path) -> bool:
    if destination.exists():
        if not destination.is_file():
            raise ValueError(f"配置模板路径不是文件：{destination}")
        return False
...
def install_application(
    ...
    """Install or upgrade the remote edition without opening GUI dialogs."""
    ...
    bundled_config_example = source_dir / "config" / CONFIG_EXAMPLE
    if not bundled_exe.is_file():
        raise FileNotFoundError(f"安装包缺少主程序：{APP_EXE}")
    if not bundled_config_example.is_file():
        raise FileNotFoundError(f"安装包缺少配置模板：{CONFIG_EXAMPLE}")
    ...
    _validate_file_paths(
        ...
        (
            Path(APP_EXE),
            Path(LAUNCHER_NAME),
            Path("config") / CONFIG_EXAMPLE,
            Path("config/mysql_config.json"),
        ),
    )
    _prepare_directories(target, DATA_DIRECTORIES + (Path("config"),))
    _atomic_copy(bundled_exe, target / APP_EXE)
    config_dir = target / "config"
    _copy_if_missing(bundled_config_example, config_dir / CONFIG_EXAMPLE)
...
class Installer(tk.Tk):
    ...
    def build(self):
        ...
        ttk.Label(
            root,
            text="请选择安装路径。升级时会更新主程序，但不会覆盖已有业务数据和数据库配置。",
            wraplength=640,
        ).pack(anchor="w", pady=(8, 12))
        ttk.Label(
            root,
            text=(
                "首次启动前，请将 config\\mysql_config.example.json 复制为 "
                "config\\mysql_config.json，并填写可用的 MySQL 连接信息。"
            ),
            foreground="#9A5B00",
            wraplength=640,
    ...
    def install(self):
        ...
        messagebox.showinfo(
            ...
            (
                f"已安装到：\n{target}\n\n"
                "首次运行前请根据 config\\mysql_config.example.json 创建并填写 "
